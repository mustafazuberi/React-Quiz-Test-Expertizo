...
const ActiveQuestion = ({ allQuestions, handleOnNext, currentQIndex, currentQuestion, handleOnOptionSelect }: Props) => {
      ...
      <section className='flex flex-col justify-start items-center gap-y-7'>
        ...
        <section className='flex flex-row gap-3 sm:w-[550px] w-[90%] min-w-full flex-wrap'>
          {options.map((option: String, i) => {
            return <button disabled={!!currentQuestion.selectedOption} key={i} className={`p-2 w-[45%] border ${currentQuestion.selectedOption === option ? 'border-black text-white bg-black' : 'text-black bg-white'}`} onClick={() => handleOnOptionSelect({ option, qIndex: currentQIndex })}>{option}</button>
          })}
        </section>
        ...
        {currentQuestion.selectedOption && <button className="p-3 bg-black text-white mb-3" onClick={() => handleOnNext()}>Next Question</button>}
      </section>
    </main>
  )
}
